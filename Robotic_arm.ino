...
struct ServoPins // Holds servo object, pin, name, and starting position
{
  Servo servo;
  int servoPin;
  String servoName;
...
// servoPins: Vector of 4 servo configurations.
...
struct RecordedStep // Holds servo index, value, and delay for recorded steps
{
  int servoIndex;
  int value;
  int delayInStep;
};
// recordedSteps: Stores recorded sequence.
std::vector<RecordedStep> recordedSteps;

// recordSteps, playRecordedSteps: Booleans to control modes.
bool recordSteps = false;
bool playRecordedSteps = false;

// previousTimeInMilli: Tracks delay between movements.
...
// WebSocket Events
// Parses key,value from UI
// Updates servo angle or toggles recording/playing
// Records steps during movement if record is ON
...
// If recording is ON:Captures current time and Stores delay and angle in recordedSteps
...
// Playback logic
// Moves servos to initial position slowly
// Replays each recorded step with original delays
// Updates WebSocket so UI reflects motion
...
void setUpPinModes() // Attaches each servo and sets to initial (90°) position
{
  for (int i = 0; i < servoPins.size(); i++)
  {
...
void setup(void) 
{ // Attaches servos to their pins
  setUpPinModes();
  Serial.begin(115200);

  // Starts the ESP32 Wi-Fi hotspot
  WiFi.softAP(ssid, password);
  IPAddress IP = WiFi.softAPIP();
  Serial.print("AP IP address: ");
  Serial.println(IP);
  
  // Initializes Web Server & WebSocket
...
// main loop
// Constantly cleans up WebSocket clients
// If playRecordedSteps == true, triggers replay
void loop() 
{
